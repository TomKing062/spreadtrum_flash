...
void set_active(spdio_t* io, char *arg)
{
	...
	char dfile[40];
	snprintf(dfile, sizeof(dfile), "misc.bin");
	if (1048576 != dump_partition(io, "misc", 0, 1048576, dfile, 0x1000)) {
		remove(dfile);
		return;
	}
	FILE* fi;
	...
	if (fseek(fi, 0x800, SEEK_SET) != 0) ERR_EXIT("fseek failed\n");
	if (fwrite(tmp, 1, 32, fi) != 32) ERR_EXIT("fwrite failed\n");
	fclose(fi);

	if (savepath[0]) load_partition(io, "misc", fix_fn, 0x1000);
	else load_partition(io, "misc", dfile, 0x1000);
}
